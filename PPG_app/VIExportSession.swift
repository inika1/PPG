            ...
            let complete = autoreleasepool(invoking: { [weak self] () -> Bool in
                ...
                if let sampleBuffer = output.copyNextSampleBuffer() {
                    ...
                    if let videoOutput = strongSelf.videoOutput, videoOutput == output {
                        self?.lastVideoSamplePresentationTime = CMSampleBufferGetPresentationTimeStamp(sampleBuffer)
                        if strongSelf.asset.duration.seconds > 0 {
                            strongSelf.videoProgress = Float(self!.lastVideoSamplePresentationTime.seconds / strongSelf.asset.duration.seconds)
                        } else {
                            strongSelf.videoProgress = 1
                        }
                    } else if let audioOutput = strongSelf.audioOutput, audioOutput == output {
                        self?.lastAudioSamplePresentationTime = CMSampleBufferGetPresentationTimeStamp(sampleBuffer)
                        if strongSelf.asset.duration.seconds > 0 {
                            strongSelf.audioProgress = Float(self!.lastAudioSamplePresentationTime.seconds / strongSelf.asset.duration.seconds)
                        } else {
